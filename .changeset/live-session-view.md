...
**Scenario-level `maxIterations`.** `ScenarioDefinition` accepts an optional `maxIterations` (positive integer). The validator rejects strings, zero, negatives, and non-integers. The CLI (`run-scenario` and `autopilot`) forwards the declared cap into `runScenario`, which writes it onto `session.start`. The metrics header renders `iterations / maxIterations` (e.g. `3 / 10`) when the scenario declares a cap, and the elapsed-only count otherwise. Display-only — runtime caps on individual `ctx.run()` calls are unchanged.
