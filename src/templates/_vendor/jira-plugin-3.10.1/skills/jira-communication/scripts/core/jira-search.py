...
LOCAL PATCH (2026-04-16): Bumped atlassian-python-api to v4+ and switched
.jql() -> .enhanced_jql() to use the new /rest/api/3/search/jql endpoint.
The legacy /search endpoint was shut off 2025-10-31.
...
def query(ctx, jql: str, max_results: int, fields: str, truncate: int | None):
    ...
    try:
        ...
        # Execute search
        results = client.enhanced_jql(jql, limit=max_results, fields=field_list)
