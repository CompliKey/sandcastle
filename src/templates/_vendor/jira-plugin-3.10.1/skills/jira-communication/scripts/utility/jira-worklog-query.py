...
LOCAL PATCH (2026-04-16): Bumped atlassian-python-api to v4+ and rewrote
search_issues() to use .enhanced_jql() with nextPageToken pagination.
The legacy /search endpoint was shut off 2025-10-31 (no more `start`/`total`).
See docs/research/2026-04-16-jira-agentic-improvements.md.
"""
...
def search_issues(client, jql: str) -> list[dict]:
    """Search issues matching JQL, paginated via /search/jql (nextPageToken/isLast).

    The new endpoint does NOT return `total`; use `isLast` + empty-page as
    stop conditions. See https://developer.atlassian.com/changelog/#CHANGE-2046
    """
    issues = []
    page_size = 50
    next_token: str | None = None
    while True:
        result = client.enhanced_jql(
            jql,
            limit=page_size,
            fields=["key", "summary"],
        ...
        if result.get("isLast", True) or not page:
            break
